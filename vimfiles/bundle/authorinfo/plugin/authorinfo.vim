...
"func:如果打开的文件类型是type，则返回1,否则返回0
...
function s:AddTitle()
    ...
    let oldline = getline('.')
    silent exec 'normal '.s:t_mapleader.'cm'
    let newline = getline('.')
    if oldline != newline
        let hasMul = 1
        let preChar = '#'
    else
        silent exec 'normal '.s:t_mapleader.'cl'
        let newline = getline('.')
        if oldline == newline
            let hasMul = -1
            let noTypeChar = '#'
    ...
    if hasMul == 1
        silent exe 'normal '.firstLine.'Gv'.lastLine.'G'.s:t_mapleader.'cm'
    else
        silent exe 'normal '.firstLine.'Gv'.lastLine.'G'.s:t_mapleader.'cl'
    endif

    silent exe 'normal '.gotoLn.'G'
    startinsert!
